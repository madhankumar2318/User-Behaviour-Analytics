...
class ReportGenerator:
    ...
    def get_report_data(self, start_date, end_date):
        ...
        stats = {
            ...
            'avg_risk_score': sum(l['risk_score'] for l in logs) / len(logs) if logs else 0
        }
    ...
    def generate_pdf_report(self, start_date, end_date, report_type='daily'):
        ...
        # Title
        title = Paragraph(f"<b>User Behavior Analytics Report</b><br/>{report_type.capitalize()} Report", 
                         styles['Title'])
        ...
        if high_risk_logs:
            elements.append(Paragraph("<b>Top 10 High-Risk Activities</b>", styles['Heading2']))
            elements.append(Spacer(1, 12))
