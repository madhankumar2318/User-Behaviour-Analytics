...
class UserProfile:
    ...
    def update_profile(self, logs):
        ...
        self.baseline["avg_failed_attempts"], _ = self.calculate_statistics(
            failed_attempts
        )
    ...
    def calculate_deviation_score(self, new_log):
        ...
        if hour_score > 15:
            score += hour_score
            reasons.append(
                f"Unusual login time ({new_hour:.1f}h vs avg {self.baseline['avg_login_hour']:.1f}h)"
            )
        ...
        if self.baseline["avg_downloads"] and self.baseline["avg_downloads"] > 0:
            ...
            if download_ratio > 2.0:
                ...
                reasons.append(
                    f"High downloads ({new_downloads} vs avg {self.baseline['avg_downloads']:.1f})"
                )
        ...
        if new_failed > self.baseline["avg_failed_attempts"] + 2:
            failed_score = min(
                15, (new_failed - self.baseline["avg_failed_attempts"]) * 5
            )
            score += failed_score
            reasons.append(
                f"High failed attempts ({new_failed} vs avg {self.baseline['avg_failed_attempts']:.1f})"
            )
    ...
    def to_dict(self):
        ...
        return {"user_id": self.user_id, "baseline": self.baseline}
