...
class VelocityChecker:
    ...
    def check_rapid_logins(
        self, user_id, current_time, history, threshold_minutes=5, threshold_count=3
    ):
        """
        ...
        recent_logins = []
        for log in history:
            time_diff = self.time_diff_minutes(
                log.get("login_time", "00:00"), current_time
            )
        ...
        if len(recent_logins) >= threshold_count:
            return (
                True,
                f"{len(recent_logins)} logins within {threshold_minutes} minutes",
            )
    ...
    def check_impossible_travel(self, current_log, last_log):
        ...
        time_diff = self.time_diff_hours(
            last_log.get("login_time", "00:00"), current_log.get("login_time", "00:00")
        )
        ...
        if required_speed > self.max_speed:
            return (
                True,
                f"Impossible travel: {distance:.0f}km in {time_diff:.1f}h (requires {required_speed:.0f}km/h)",
            )

        return False, None

    def check_concurrent_sessions(
        self, user_id, current_time, history, threshold_minutes=30
    ):
        """
        ...
        if len(recent_locations) > 1:
            return (
                True,
                f"Concurrent sessions from {len(recent_locations)} locations: {', '.join(recent_locations)}",
            )
    ...
    def perform_all_checks(self, current_log, user_history):
        ...
        # Check rapid logins
        is_rapid, rapid_msg = self.check_rapid_logins(
            user_id, current_time, user_history
        )
        if is_rapid:
        ...
        if user_history and len(user_history) > 0:
            ...
            is_impossible, impossible_msg = self.check_impossible_travel(
        ...
        is_concurrent, concurrent_msg = self.check_concurrent_sessions(
            user_id, current_time, user_history
        )
        if is_concurrent:
        ...
        return {"has_alerts": len(alerts) > 0, "alerts": alerts, "severity": severity}
